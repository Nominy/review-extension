...
test('paid calls read the latest separate key and reject missing keys and untrusted origins', async () => {
  ...
  try {
    await assert.rejects(keyedRequest('https://reviewgen.ovh', '/api/review/generate', {}));
    assert.equal(requests.length, 0, 'missing credentials must prevent a backend request');
